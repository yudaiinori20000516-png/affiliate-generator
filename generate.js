...
function loadStrategyForTitle(title) {
...
async function generateArticle(title) {
  const strategy = loadStrategyForTitle(title);
  const strategyBlock = strategy
    ? `\n\n【Claude Code指示役による売れる記事設計】\n以下の設計を最優先で反映してください。ただし本文に「戦略」「設計シート」などのメタ表現は出さないでください。\n\n${strategy}`
    : "";
...
価格・有料記事設計:
- この記事は100円の有料記事として設計する
- 記事全体の約70%までは無料部分として、読者が「ここまででも価値がある」と感じる内容を書く
- 約70%地点に必ず以下の行をそのまま入れる
【ここから有料部分｜100円】
- 有料部分には、実際に使えるプロンプト集・テンプレート・チェックリストを入れる
- 有料部分は出し惜しみせず、「これだけで100円は安すぎる」と思わせる具体性にする
- 無料部分の最後では、売り込みではなく「ここから先は実際に使える形でまとめます」という自然な流れで有料部分へつなぐ

記事の質の要件:
- 文字数は3000〜4000字
- 書籍1冊分の価値があるレベルの情報密度を目指す
- 抽象論だけで終わらせず、具体例・手順・数字・失敗談を必ず入れる
- 実体験を必ず入れる。例: 夜勤明けに作業した話、最初に失敗した話、収益や作業時間の数字、投稿後の反応など
- 読者が明日から実践できる具体的なステップを入れる
- 「何を」「どの順番で」「どれくらいの時間で」「どう判断するか」まで書く
- 読者が読み終わったあとに、最初の1アクションを迷わない状態にする

注釈の要件:
- 専門用語や難しい概念が出てきたら、本文中に「※用語とは：説明」の形式で注釈を入れる
- 例: 「※Claude Codeとは：Anthropicが開発したAIコーディングツール」
- 記事末尾に【注釈一覧】を作り、本文中で使った注釈をまとめる
- 注釈は初心者が置いていかれないための補足として、短くわかりやすく書く

図解の要件:
- 複雑な概念や作業フローを説明する箇所には、テキスト図解を入れる
- ASCIIアートや矢印を使って、スマホでも読める形にする
- 例:
記事ネタ作成 → 本文生成 → リンク挿入 → 投稿
        ↓
   X・Threads・Noteへ展開
- 図解の前後には、何を表している図なのかを短く説明する

構成の要件:
- 導入: 読者の悩みに深く共感する。約300字
- 本編: 具体的なノウハウを段階的に説明する。約2000字
- 図解・注釈: 理解を深める補足を入れる。約500字
- まとめ: 明日できるアクションを提示する。約300字
- 有料部分: 実際に使えるプロンプト集・テンプレート・チェックリストを入れる
- 商品・サービスを自然な流れで紹介する
- アフィリエイトリンクを挿入すべき箇所に【LINK】マーカーを入れる（2〜4箇所）
- 【LINK】の前に楽天検索用のコメントを入れる（例: <!-- ChatGPT 仕事術 本 -->【LINK】）
- 楽天検索用コメントには、実在の商品カテゴリや本の検索キーワードだけを書く
- 「有料note誘導」「Claude Pro公式プラン」など、楽天で商品検索できないキーワードは使わない
...
- テンプレっぽい定型文にしない。同じような誘導・同じような締め方を避ける
- 1文は短くする。1行あたり20〜35文字くらいを目安にする
- 改行を多めに入れて、スマホで縦に読みやすくする
- 空白行は使ってよいが、連続する空白行は入れない

投稿の構成（1投稿に全部収める）:
1. 冒頭1行目に強いフックを入れる
   - 疑問、違和感、失敗談、意外な気づき、数字のどれかを使う
   - ただし煽りすぎない
2. 2行目以降で、なぜそう思ったのかを体験談として短く書く
3. 読者が「自分にも関係ある」と思う気づきを1つ入れる
4. Noteへの誘導は自然に1行だけ入れる。毎回同じ表現にしない
5. 最後の行にハッシュタグを3個だけ付ける

ルール:
- 280文字以内厳守
- URLは入れない
- ハッシュタグは必ず3個だけ
- 使うハッシュタグは #ClaudeCode #AI副業 #副業術 の3つで固定
- 広告・誘導っぽい表現禁止
- 「詳しくはプロフのNoteに」「気になった人はNoteも見てみて」などのテンプレ文をそのまま使わない
- 「〇〇で悩んでいませんか？」から始めない
- 説明しすぎず、投稿単体でも読める余韻を残す
